...
export const ExecuteBoomerangCommand = async ({
    ...
    await ffmpeg.run(
        '-ss', getFFMpegTimestampFromSeconds(startTime),
        '-to', getFFMpegTimestampFromSeconds(startTime + duration),
        '-i', videoMetadata.name,
        '-filter_complex', `[0]scale=-2:${targetHeight},setsar=1:1[s];[s]split[s1][s2];[s1]reverse[r];[s2][r]concat=n=2[out];`,
        '-map', '[out]', OUTPUT_FILE_NAME
    );
